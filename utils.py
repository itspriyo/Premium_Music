...
RADIO_TITLE=os.environ.get("RADIO_TITLE", "🎶 Music 24/7 | Love Radio")
if RADIO_TITLE=="NO":
    RADIO_TITLE = None
...
class MusicPlayer(object):
    ...
    async def c_play(self, channel):
        ...
        try:
            ...
            if not playlist:
                print("No songs Found From Channel, Starting Love FM")
                Config.CPLAY=False
                Config.STREAM_URL="https://mirchilove-lh.akamaihd.net/i/MIRCHILOVE_1@32886/index_1_a-p.m3u8"
            ...
            for play in f:
                ...
                if len(playlist) == 1:
                    ...
                    if EDIT_TITLE:
                        await self.edit_title()
                if not playlist:
                    print("Invalid Playlist File, Starting LoveFM")
                    Config.YPLAY=False
                    Config.STREAM_URL="https://mirchilove-lh.akamaihd.net/i/MIRCHILOVE_1@32886/index_1_a-p.m3u8"
                    await self.start_radio()
                    file.close()
